...
void Timer0_Routine() interrupt 1	  //定时器中断
{
	...
	T0count1++;
	if(T0count2 >= 10)	  //每20ms调用一次Matrix_Loop
	{
		T0count2 = 0;
		Matrix_Loop();
	}
...
//		UART_SendByte(0x35);
		...
		{
			Flag += 1;
			Flag %= 3;//Flag越界判断，若大于等于3，则赋值0；
		}
		else if(KeyNum == 2)	  //按下键2，则进行  +
		{
			if(Flag == 0)		  //Flag值选择调节的位
			{
				Sec++;
				Sec %= 60;
			}
